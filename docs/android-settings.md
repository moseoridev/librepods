...
This is a **partial visual migration**, not Buds pixel parity or full iOS feature parity. On the connected Home page the original and this implementation were rendered on the same device and the two captures differ on 0.42% of pixels (light) and 0.47% (dark), with the residual concentrated in antialiasing, a vendor-only card divider and toolbar icons the comparison fixture does not draw; the captures and the comparison are held outside the repository. Battery artwork, EQ, dialogs and the remaining screens still need dedicated comparison. The shared menu, section and choice components now follow the original common renderer; specialized controls and complete page composition are not all verified. No One UI parity, jank or power measurements establish this change's effects. The supplied iOS hierarchy is also partial: volume, charging notifications, case sounds and Apple-service destinations are not added by this move, and existing connection/EQ/head-tracking behavior is not redefined to claim iOS equivalence.
...
`ku.i0.N → wu.x/wu.u` defines the 32×20dp switch and its 16dp thumb with 2dp padding. Track and padded thumb are measured independently before placement; at fractional densities their rounded pixel heights can differ. Thumb travel uses the measured widths and integer placement, with `placeRelative` preserving RTL. The One UI 8+ default path uses the primary blue checked track and bright thumb in both themes; the inverse palette belongs to the older `up.e.O()` branch. The track follows the checked state alone — `sesl_switch_track_off_color` (outline) to `sesl_switch_track_on_color` (primary) as the thumb travels — and disabling swaps both tokens for their `_on_disabled_`/`_off_disabled_` members, which are those same two colours at 40%, so the hue still reports the state. That 40% is the token's own opacity and is not the whole story: the row dims the finished control by a further 40%, and the two multiply. The switch therefore composites as one layer rather than fading each colour again, which is what puts a disabled checked track at 16% and leaves the thumb a lighter disc over it rather than a hole through to the card. The containing row owns the accessible toggle. A bounded 200ms transition remains an adaptation; custom Samsung theme drawables and their drag/press effects are not reproduced.
...
The visible scaffold composites its transparent content over the page background before applying system/header padding. A locally authored linear top fade runs over 56dp from 12% to full content opacity; the compact header background is then drawn above it. Moving the effect to an individual card, painting an opaque detail background inside it, or offsetting only title text changes the result. The shader and effect are remembered by color and density. This requires one viewport-sized foreground offscreen layer, but introduces no frame clock, timer or background work. GPU memory/scrolling cost remains unmeasured.

The bottom edge reproduces the One UI fading edge instead of a second linear ramp. `ou.g` gives it a 70dp total height, an 18dp lower zone and a 0.24 alpha at that zone's upper bound, and `m3.i.Q` builds the three-stop profile from those: alpha falls linearly from 1 to 0.24 over the 52dp above the zone, then continues down through the zone to 0 at the edge. The 0.24 is a waypoint rather than a floor — the original does fade the last stretch to nothing, which the measured profile confirms (the vendor's own lowest text row is drawn at 0.12 alpha, half of 0.24). The original enables the bottom stops from the content's scroll state (`gm.w2` reads `ScrollState.d()`), so the effect is gated the same way here: `StyledScaffold` owns a `BudsScrollState` holder and a scrolling screen reports into it through `BudsScrollReporter`, which is what turns the edge on while content remains below. A screen that does not scroll gets no bottom edge. Samsung window/custom-theme effects are still not reproduced.
...
The zero-length active segment is skipped at the low endpoint, because a round-capped `drawLine` with identical endpoints still paints a dot where the native `ClipDrawable` paints nothing at level 0. Verified by reading pixels on the API 37 emulator in both themes and under an RTL locale: 8px (3dp at density 420) released and 34px pressed, no gap between the active track and the thumb ring, the correct per-theme thumb core, the translucent inactive track over the card, and the fill growing leftwards from the right edge while the thumb travels right to left. Not verified: the native 62ms-stepped `PathInterpolator` is not reproduced (the expansion is a plain 250ms `tween`), the thumb's 100ms press-in / 300ms release-inner-circle animation is not implemented, TalkBack behavior of the new renderer is unexercised, the disabled and maximum-value states were not captured, and no Samsung-device or per-call-site Buds screenshot comparison exists. The EQ screen has no slider at all — iOS shows a LOW/MID/HIGH graph there and LibrePods presents no editable EQ — and the quick-settings dialog's `VerticalVolumeSlider` is a separate, uncompared renderer.
