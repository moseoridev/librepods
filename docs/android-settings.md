...
The listening-mode card now uses a single Compose renderer instead of the old Apple drag selector and Material segmented buttons. ASC `gl.s` supplies the 44dp option/caption layout (caption origin 52dp, 12sp text); `NoiseControlSeekBarBackgroundView` supplies the 6dp joining line, with `color_noise_controls_progress_tint_color` and `basic_token_sys_surface_bright_dif` providing the light/dark track and selected colors. The implementation keeps iOS ordering and LibrePods icons. It uses click/radio semantics and renders the existing UiState, which the ViewModel updates optimistically after sending a command; selection does not guarantee an AirPods acknowledgement. No proprietary Lottie is included, and the original native seekbar/animation is not reproduced. Missing/invalid ANC values leave all modes unselected.
